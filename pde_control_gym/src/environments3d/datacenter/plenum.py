...
class PlenumResult:
    ...
    tile_ix: np.ndarray
    tile_iy: np.ndarray
    Q_m3s: np.ndarray  # per-tile flow
    V_ms: np.ndarray  # per-tile approach velocity
    p_kin: np.ndarray  # per-tile shifted kinematic pressure
    prd_m_pct: np.ndarray  # Eq. 16 percent deviation from mean flow
    prd_max: float
    prd_min: float
    prd_std: float
    steps: int
